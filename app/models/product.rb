class Product < ApplicationRecord
  ...
  has_many :stock_receipt_lines, dependent: :restrict_with_error
  ...
  validates :reorder_point, numericality: { greater_than_or_equal_to: 0, only_integer: true }

  scope :active, -> { where(active: true) }
  scope :orderable, -> { active.where("stock > 0") }
  scope :by_item_number, -> { order(:item_number) }
  scope :low_stock, -> { active.where("stock <= reorder_point").order(:stock) }
